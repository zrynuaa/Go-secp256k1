...
import (
	...
	"crypto/sha256"
	"encoding/hex"
	secp "github.com/zrynuaa/Go-secp256k1/secpk1"
	"math/big"
	"testing"
)
...
func TestKeyGeneration(t *testing.T) {
	testKeyGeneration(t, secp.S256(), "S256")
	if testing.Short() {
		return
	}
	testKeyGeneration(t, secp.S160(), "S160")
	testKeyGeneration(t, secp.S192(), "S192")
	testKeyGeneration(t, secp.S224(), "S224")
}

//直接调用官方库实现的ECDSA标准算法，传入secp256k1曲线参数，就可以签名验签
...
func TestSignAndVerify(t *testing.T) {
	testSignAndVerify(t, secp.S256(), "S256")
	if testing.Short() {
		return
	}
	testSignAndVerify(t, secp.S160(), "S160")
	testSignAndVerify(t, secp.S192(), "S192")
	testSignAndVerify(t, secp.S224(), "S224")
}
...
func TestVectors(t *testing.T) {
	...
	for i, test := range testVectors {
		...
		r := fromHex(test.r)
		s := fromHex(test.s)
		if ecdsa.Verify(&pub, hashed, r, s) != test.ok {
